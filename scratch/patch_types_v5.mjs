...
// Remove existing entity_job_offers table if it exists. We match the block by counting braces if possible, or using a safer regex.
// A simpler way is to just replace the whole text matching the indentation.
content = content.replace(/\\s+entity_job_offers: \\{[\\s\\S]*?Relationships: \\[[\\s\\S]*?\\]\\n\\s+\\},/g, '')

// Inject tables into the first occurrence of `    Tables: {`
content = content.replace('    Tables: {', '    Tables: {\\n' + tablesToInject)

// Inject enums into the first occurrence of `    Enums: {`
