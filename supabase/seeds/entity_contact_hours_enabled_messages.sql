-- Seed manuel : toggle horaires + table entity_messages
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'entity_messages' AND policyname = 'entity_messages_insert_public') THEN
    CREATE POLICY "entity_messages_insert_public" ON entity_messages FOR INSERT
      WITH CHECK (
        EXISTS (
  ...
  END IF;
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'entity_messages' AND policyname = 'entity_messages_owner_select') THEN
    CREATE POLICY "entity_messages_owner_select" ON entity_messages FOR SELECT
      USING (
        EXISTS (
