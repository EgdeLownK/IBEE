...
export async function POST(request: Request) {
  try {
    ...
    if (chargeCents <= 0) {
      return NextResponse.json(
        { error: 'Ce service ne nécessite pas de paiement en ligne.' },
        { status: 400 },
      )
    }
    ...
    const bookingPath = slug ? `/${slug}/services/${svcSlug}/booking` : '/'
    const confirmedPath = slug ? `/${slug}/services/${svcSlug}/confirmed` : '/'
